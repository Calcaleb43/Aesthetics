...
export function PageHero({
  ...
  const height =
    size === "tall" ? "min-h-[78vh]" : size === "compact" ? "min-h-[46vh]" : "min-h-[58vh]";

  return (
    <section className={`relative overflow-hidden bg-black text-white ${height}`}>
      ...
      <div
        className={`relative z-10 mx-auto flex ${height} max-w-7xl flex-col justify-end px-5 pb-12 pt-28 lg:px-8 lg:pb-16 ${
      ...
      >
        <div className={`fade-up ${align === "center" ? "max-w-3xl" : "max-w-4xl"}`}>
          ...
          <h1 className="display fade-up-delay-1 mt-4 text-[clamp(2.6rem,7vw,5.6rem)] leading-[0.92]">
            {title}
          </h1>
          <div
            className={`reveal-line mt-6 h-px w-40 bg-[linear-gradient(90deg,var(--gold),transparent)] ${
              align === "center" ? "mx-auto" : ""
            }`}
          />
          {subtitle ? (
            <p className="fade-up-delay-2 mt-6 max-w-2xl text-lg leading-8 text-white/80 md:text-xl">
              {subtitle}
            </p>
          ) : null}
          {ctas && ctas.length > 0 ? (
