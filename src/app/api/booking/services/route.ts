...
export async function GET() {
  if (!hasDatabase()) {
    return NextResponse.json(
      { enabled: false, categories: [], error: "Booking requires DATABASE_URL or POSTGRES_URL on this environment" },
      { status: 503 },
    );
  }
  ...
  if (!settings.bookingEnabled) {
    return NextResponse.json({ enabled: false, categories: [], bookingUrl: settings.bookingUrl });
  }
  ...
  try {
    ...
    return NextResponse.json(
      { enabled: false, categories: [], error: "Could not load bookable services" },
      { status: 503 },
    );
  }
}
